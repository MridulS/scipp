...
void ElementTypeMap<scipp::core::time_point>::check_assignable(
  ...
  // accessor that borrows the temporary array, which dies at the end of
  // this statement.
...
scipp::core::time_point make_time_point(const nb::object &buffer,
                                        const int64_t scale) {
  // astype circumvents numpy.datetime64.__int__, which delegates to
  // datetime.datetime if the unit is larger than ns and that cannot be
  // converted to long.
  using PyType = typename ElementTypeMap<core::time_point>::PyType;
  return core::time_point{nb::cast<PyType>(buffer.attr("astype")("int64")) *
                          scale};
}
