...
template <class RHSSetup> struct OpBinder {
  ...
  static void in_place_binary(nanobind::class_<T, Ignored...> &c) {
    ...
    // In-place operators return nb::object due to the way in-place operators
    ...
    // WARNING: It is crucial to explicitly return 'nb::object &' here.
    // Otherwise the nb::object is returned by value, which increments the
    // reference count, which is not only suboptimal but also incorrect since
    // we have released the GIL via nb::gil_scoped_release.
    c.def(
        "__iadd__",
        [](nb::object &a, Other &b) -> nb::object & {
          nb::cast<T &>(a) += RHSSetup{}(b);
          return a;
        },
        nb::is_operator(), nb::call_guard<nb::gil_scoped_release>());
    c.def(
    ...
    if constexpr (!(std::is_same_v<T, Dataset> ||
      ...
      c.def(
          "__ifloordiv__",
          [](nb::object &a, Other &b) -> nb::object & {
            floor_divide_equals(nb::cast<T &>(a), RHSSetup{}(b));
            return a;
          },
          nb::is_operator(), nb::call_guard<nb::gil_scoped_release>());
...
void bind_logical(nanobind::class_<T, Ignored...> &c) {
  ...
  c.def(
      "__ior__",
      [](const nb::object &a, const T2 &b) -> const nb::object & {
        nb::cast<T &>(a) |= b;
        return a;
      },
      nb::is_operator(), nb::call_guard<nb::gil_scoped_release>());
