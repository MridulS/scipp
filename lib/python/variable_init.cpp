...
bool is_empty(const nb::object &sequence) {
  if (PyObject_CheckBuffer(sequence.ptr()) != 0) {
    return nb::cast<scipp::index>(sequence.attr("ndim")) == 0;
  }
  return !nb::bool_{sequence};
}

nb::object numpy_asarray(const nb::object &obj) {
  return nb::module_::import_("numpy").attr("asarray")(obj);
}
...
nb::object parse_data_sequence(const nb::object &dim_labels,
  ...
  // Need to check for None because np.asarray does not preserve it.
  if (is_empty(dim_labels) || data.is_none()) {
    return data;
  } else {
    return numpy_asarray(data);
  }
}
...
T extract_scalar(const nb::object &obj, const sc_units::Unit unit) {
  ...
  if (PyObject_CheckBuffer(obj.ptr()) != 0) {
...
core::time_point extract_scalar<core::time_point>(const nb::object &obj,
  ...
  if (PyObject_CheckBuffer(obj.ptr()) != 0) {
    ensure_is_scalar(obj);
    // nanobind has no np.dtype wrapper; numpy accepts a dtype string.
    return core::time_point{
        nb::cast<PyType>(obj.attr("astype")("int64").attr("item")())};
  } else {
...
Variable make_structured_variable(const nb::object &dim_labels,
  ...
  const auto values = numpy_asarray(values_);
...
void bind_init(nb::class_<Variable> &cls) {
  cls.def(
      ...
      nb::arg("variances") = nb::none(), nb::arg("unit").none() = DefaultUnit{},
