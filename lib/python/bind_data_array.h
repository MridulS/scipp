...
void bind_helper_view(nb::module_ &m, const std::string &name) {
  ...
  auto cls =
      nb::class_<View<T>>(m, (name + suffix).c_str())
          ...
          .def(
              "__iter__",
              [iter_name = name + suffix + "_iterator"](const View<T> &self) {
                // The element rv_policy must be the template argument; a
                // runtime rv_policy extra would be silently ignored by
                // nb::make_iterator. Use `copy` because elements may
                // reference the underlying dict (nanobind would turn
                // reference policies into moves for pair elements, gutting
                // the dict). This matches __getitem__, which also returns
                // (shallow) copies.
                return nb::make_iterator<nb::rv_policy::copy>(
                    nb::type<View<T>>(), iter_name.c_str(), self.begin(),
                    self.end());
              },
              nb::keep_alive<0, 1>());
  if constexpr (!std::is_same_v<View<T>, values_view<T>>) {
...
void bind_common_mutable_view_operators(nanobind::class_<T, Ignored...> &view) {
  view.def("__len__", &T::size)
      ...
      .def("__contains__", [](const T &self, const nb::handle &key) {
        std::string str_key;
        if (!nb::try_cast<std::string>(key, str_key))
          return false; // if `key` is not a string, it cannot be contained
        return self.contains(typename T::key_type{str_key});
      });
}
...
void bind_mutable_view(nb::module_ &m, const std::string &name,
  ...
  view.def(
          "__iter__",
          [iter_name = name + "_iter"](const T &self) {
            ...
            return nb::make_iterator<nb::rv_policy::move>(
                nb::type<T>(), iter_name.c_str(), self.keys_begin(),
                self.keys_end());
          },
          nb::keep_alive<0, 1>())
      .def(
...
void bind_mutable_view_no_dim(nb::module_ &m, const std::string &name,
  ...
  view.def(
          "__iter__",
          [iter_name = name + "_iter"](T &self) {
            auto keys_view = str_keys_view(self);
            // The element rv_policy must be the template argument; a runtime
            // rv_policy extra would be silently ignored by nb::make_iterator.
            return nb::make_iterator<nb::rv_policy::move>(
                nb::type<T>(), iter_name.c_str(), keys_view.begin(),
                keys_view.end());
          },
          nb::keep_alive<0, 1>())
      .def(
...
The name is preserved through operations:
  ...
  c.def_prop_rw(
      ...
      nb::for_getter(nb::rv_policy::copy),
