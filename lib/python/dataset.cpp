...
void bind_dataset_coord_properties(nb::class_<T, Ignored...> &c) {
  // nanobind property getters default to rv_policy::reference_internal,
  // which keeps `self` alive while the returned dict view exists.
  c.def_prop_ro(
...
void bind_dataset_view_methods(nb::class_<Dataset, Ignored...> &c) {
  ...
  c.def(
      "__iter__",
      [](const Dataset &self) {
        // The element rv_policy must be the template argument; a runtime
        // rv_policy extra would be silently ignored by nb::make_iterator.
        return nb::make_iterator<nb::rv_policy::move>(
            nb::type<Dataset>(), "Dataset_keys_iterator", self.keys_begin(),
            self.keys_end());
      },
      nb::keep_alive<0, 1>());
  c.def(
...
Access a data item in the dataset:
  ...
  c.def("__contains__", [](const Dataset &self, const nb::handle &key) {
    std::string name;
    // if `key` is not a string, it cannot be contained
...
// Convert a Mapping or iterable of (key, value) pairs to a Python dict,
// replicating pybind11's py::dict(obj) converting constructor.
nb::dict to_pydict(const nb::object &obj) {
  if (PyDict_Check(obj.ptr()))
    return nb::borrow<nb::dict>(obj); // reuse the same dict, no copy
  // dict(obj) accepts a Mapping or an iterable of (key, value) pairs and
  // raises TypeError otherwise, which propagates as nb::python_error.
  const nb::handle dict_type(reinterpret_cast<PyObject *>(&PyDict_Type));
  return nb::cast<nb::dict>(dict_type(obj));
}

template <class Key, class Value> auto to_cpp_dict(const nb::object &obj) {
  const auto dict = to_pydict(obj);
...
scipp.Variable, scipp.Dataset
  ...
  dataArray.def(
      ...
      nb::sig("def __init__(self, data: Variable, *, "
              "coords: Union[Mapping[str, Variable], "
              "Iterable[tuple[str, Variable]]] = {}, "
              "masks: Union[Mapping[str, Variable], "
              "Iterable[tuple[str, Variable]]] = {}, "
              "name: str = '') -> None"),
      R"doc(DataArray initializer.
...
scipp.DataArray, scipp.Variable
  ...
  dataset.def(
      ...
      [](Dataset *self, const nb::object &data, const nb::object &coords) {
        ...
        const auto data_dict = data.is_none() ? nb::dict() : to_pydict(data);
        const auto coords_dict =
            coords.is_none() ? nb::dict() : to_pydict(coords);
      ...
      nb::sig("def __init__(self, "
              "data: Union[Mapping[str, Union[Variable, DataArray]], "
              "Iterable[tuple[str, Union[Variable, DataArray]]]] = {}, *, "
              "coords: Union[Mapping[str, Variable], "
              "Iterable[tuple[str, Variable]]] = {}) -> None"),
      R"doc(Dataset initializer.
