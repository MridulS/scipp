...
auto from_py_slice(const T &source,
  ...
  // nb::slice::compute throws nb::python_error internally on failure.
...
template <class T> struct slicer {
  ...
  static void set(T &self, const IndexOrRange &index, const nb::object &data) {
    ...
    oss << "Cannot to assign a " << nb::str(data.type()).c_str()
        << " to a slice of a " << nb::type_name(nb::type<T>()).c_str();
...
void bind_slice_methods(nanobind::class_<T, Ignored...> &c) {
  ...
  // Note the order of overloads: nanobind's two-pass dispatch (exact matches
  // first, then implicit conversions) makes the order less load-bearing than
  // with pybind11, where the std::tuple caster could call `len()` on
  // __getitem__ arguments (failing for scalar variables). We keep this
  // overload before the tuple-accepting ones as defense-in-depth.
  c.def(
      "__getitem__",
  ...
  // These overloads are also placed before the ones accepting std::tuple.
  // With pybind11 this was required because it would call `int()` on the 2nd
  // tuple element because of `Variable.__int__`; nanobind only attempts such
  // conversions in its second dispatch pass, so the tuple<string, Variable>
  // overload matches first, but we keep the conservative order anyway.
  //
